...
(* This file defines the "other" avail predicate othermbavail_pq_set, 
	 where q is the receiver whose mailbox we are checking, 
...
(* The second disjunct enumerates transitions where the sender is blocked, and the signature is not _->q *)
let filter_transitions_disjunct_two (ls: symbolic_transition list) (bs: participant list) (p: participant) (q: participant) : symbolic_transition list =
   List.filter (fun tr -> List.mem tr.sender bs && tr.receiver <> q) ls 
...
(* The third disjunct enumerates transitions where the sender is not blocked, and the signature is not _->q *)
...
let filter_transitions_disjunct_three (prot: symbolic_protocol) (ls: symbolic_transition list) (bs: participant list) (p: participant) (q: participant) : symbolic_transition list =
   List.filter (fun tr -> sender_not_blocked prot bs tr && tr.receiver <> q) ls 
...
(* The fourth disjunct enumerates all transitions with the signature _->q, when the sender is not blocked *)
let filter_transitions_disjunct_four (ls: symbolic_transition list) (bs: participant list) (p: participant) (q: participant) = 
	...
	List.filter (fun tr -> not (List.mem tr.sender bs) && tr.receiver = q) ls 
...
let generate_mbavail_for_participant_pair_and_blocked_set (prot: symbolic_protocol) (bs: participant list) (p: participant) (q: participant) : string = 
  "othermbavail_" ^ q ^ "_" ^ 
...
let generate_mbavail_for_participant_pair (prot: symbolic_protocol) (p: participant) (q: participant) : string = 
	let bs_list = all_participant_subsets_containing_participant prot q in 
	List.fold_left (fun acc bs -> acc ^ "\n" ^ generate_mbavail_for_participant_pair_and_blocked_set prot bs p q) "" bs_list
