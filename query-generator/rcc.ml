...
let generate_rcc_preamble_from_pair_for_participant_va (prot: symbolic_protocol) (pair: symbolic_transition * symbolic_transition) (p: participant) = 
	"exists " ^ 
	(* "(s1: int) (s'1: int) " ^  *)
...
let generate_rcc_body_from_pair_for_participant_va (prot: symbolic_protocol) (pair: symbolic_transition * symbolic_transition) (p: participant) = 
	"(" ^ 
	(* first_conjunct_from_pair prot pair p ^  *)
	(* " /\\ \n" ^ *)
	second_conjunct_from_pair_va prot pair p ^ 
...
let generate_rcc_preamble_for_participant_va (prot: symbolic_protocol) (p: participant) (ls : (symbolic_transition * symbolic_transition) list) = 
	generate_rcc_first_line_for_participant prot p ^ 
	List.fold_left (fun acc pair -> acc ^ "\n\\/\n" ^ generate_rcc_body_from_pair_for_participant_va prot pair p) "false" ls ^ 
	"\ns.t.\n"
...
let generate_rcc_queries_for_participant_va (prot: symbolic_protocol) (p: participant) (dir: string) =
	...
	then (let filename = p ^ "_rcc.hes" in 
			write_to_file (Filename.concat dir filename) (generate_rcc_for_participant_va prot p transition_pairs))
	else ()

let generate_rcc_queries_v1a (prot: symbolic_protocol) (dir: string) = 
  let participants = get_receivers prot in 
  List.iter (fun p -> generate_rcc_queries_for_participant_va prot p dir) participants 
...
let generate_rcc_queries_v1b (prot: symbolic_protocol) (dir: string) = 
  let participants = get_receivers prot in 
...
let generate_rcc_queries_naive (prot: symbolic_protocol) (dir: string) = 
  generate_rcc_queries_v1a prot dir 

let generate_rcc_queries_opt (prot: symbolic_protocol) (dir: string) = 
  generate_rcc_queries_v2b prot dir 
