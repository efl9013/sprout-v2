...
(* Defining stcon, i.e. st-connectivity from s1 to s2 *)
(* Either s1 has already reached s2, and the register values are the same *)
let first_disjunct_stcon (prot: symbolic_protocol) (s1: state) (s2: state) : string = 
  "(" ^ 
...
let second_disjunct_stcon_for_transition (prot: symbolic_protocol) (tr: symbolic_transition) (s1: state) (s2: state) : string = 
...
let filter_transitions_second_disjunct_stcon (ls: symbolic_transition list) (s2: state) =
   List.filter (fun tr -> tr.post = s2) ls 

let second_disjunct_stcon (prot: symbolic_protocol) (s1: state) (s2: state) : string = 
  let transition_pairs = filter_transitions_second_disjunct_stcon prot.transitions s2 in 
  List.fold_left (fun acc x -> acc ^ " \\/ \n" ^ second_disjunct_stcon_for_transition prot x s1 s2) "false" transition_pairs 

let stcon_body (prot: symbolic_protocol) (s1: state) (s2: state) : string = 
  "(\n" ^ 
  "s1 = " ^ string_of_int s1 ^ " " ^
  " /\\ " ^
  "s2 = " ^ string_of_int s2 ^ " " ^
  " /\\ " ^
  "(" ^
  first_disjunct_stcon prot s1 s2 ^ 
  "\\/ " ^
  second_disjunct_stcon prot s1 s2 ^
  "))"

let generate_stcon (prot: symbolic_protocol) : string = 
  "stcon (s1: int) " ^ 
  List.fold_left (fun acc r -> acc ^ "(" ^ r ^ "'1: int) ") "" prot.registers ^ 
  "(s2: int) " ^ 
  List.fold_left (fun acc r -> acc ^ "(" ^ r ^ "'2: int) ") "" prot.registers ^ 
  ": bool =mu\n " ^ 
  List.fold_left (fun acc (s1, s2) -> acc ^ " \\/ " ^ stcon_body prot s1 s2) "false" (all_state_pairs prot.states) ^ 
  ";"
